...
@section('subtitle', 'Kelola permintaan penukaran reward dari member')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                           placeholder="Cari berdasarkan nama member, NIS, nama reward, atau kode..."
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
...
@push('scripts')
<script>
    const rejectUrlTemplate = @json(route('admin.requests.reject', ['rewardRequest' => '__ID__']));

    function showRejectModal(requestId) {
        const form = document.getElementById('rejectForm');
        form.action = rejectUrlTemplate.replace('__ID__', requestId);
